...
func NewUnifiedLogBrt(level domain.Level, sinks ...port.Sink) *UnifiedLogBrt {
    if len(sinks) == 0 { sinks = []port.Sink{&defaultStdoutSink{}} }
    return &UnifiedLogBrt{level: level, sinks: sinks, fields: make(map[string]interface{}, 8), logCount: &atomic.Int64{}}
}

func (l *UnifiedLogBrt) F(key string, value interface{}) LogBrt { c := l.clone(); c.fields[key] = value; return c }
func (l *UnifiedLogBrt) Fs(fields map[string]interface{}) LogBrt {
    if len(fields) == 0 { return l }
    c := l.clone(); for k, v := range fields { c.fields[k] = v }; return c
}
func (l *UnifiedLogBrt) Ctx(ctx context.Context) LogBrt {
    if ctx == nil { return l }
    c := l.clone(); c.fields["context"] = ctx
    if v := util.GetTraceID(ctx); v != "" { c.fields["trace_id"] = v }
    if v := util.GetUserID(ctx); v != "" { c.fields["user_id"] = v }
    if v := util.GetRequestID(ctx); v != "" { c.fields["request_id"] = v }
    return c
}
...
func (l *UnifiedLogBrt) WithError(err error) LogBrt { if err == nil { return l }; return l.F("error", err.Error()) }
...
func (l *UnifiedLogBrt) Debugf(format string, args ...interface{}) { l.log(domain.DebugLevel, appendf(format, args...)) }
func (l *UnifiedLogBrt) Infof(format string, args ...interface{})  { l.log(domain.InfoLevel, appendf(format, args...)) }
func (l *UnifiedLogBrt) Warnf(format string, args ...interface{})  { l.log(domain.WarnLevel, appendf(format, args...)) }
func (l *UnifiedLogBrt) Errorf(format string, args ...interface{}) { l.log(domain.ErrorLevel, appendf(format, args...)) }
func (l *UnifiedLogBrt) Fatalf(format string, args ...interface{}) { l.log(domain.FatalLevel, appendf(format, args...)) }

func (l *UnifiedLogBrt) LogCount() int64 { if l.logCount == nil { return 0 }; return l.logCount.Load() }
...
func (l *UnifiedLogBrt) clone() *UnifiedLogBrt {
    ...
    for k, v := range l.fields { c.fields[k] = v }
...
func (l *UnifiedLogBrt) log(level domain.Level, msg string) {
    ...
    if len(l.fields) == 0 { // no-fields fast path (avoid map alloc)
        ...
        for _, s := range l.sinks { if s != nil { _ = s.Write(entry) } }
        if l.logCount != nil { l.logCount.Add(1) }
        return
    }
...
func newLogEntry(level domain.Level, msg string, baseFields map[string]interface{}) *domain.LogEntry {
    ...
    for k, v := range baseFields { entry.F[k] = v }
    return entry
}

// Helpers for cross‑package usage
func (l *UnifiedLogBrt) FieldsCopy() map[string]interface{} { out := make(map[string]interface{}, len(l.fields)); for k, v := range l.fields { out[k] = v }; return out }
func (l *UnifiedLogBrt) WithOnlyFields(m map[string]interface{}) *UnifiedLogBrt { c := l.clone(); c.fields = make(map[string]interface{}, len(m)); for k, v := range m { c.fields[k] = v }; return c }
func (l *UnifiedLogBrt) Level() domain.Level { return l.level }
func (l *UnifiedLogBrt) IncCount()           { if l.logCount != nil { l.logCount.Add(1) } }
func (l *UnifiedLogBrt) WriteEntry(entry *domain.LogEntry) { for _, s := range l.sinks { if s != nil { _ = s.Write(entry) } } }
