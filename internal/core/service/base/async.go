...
func (ap *AsyncPipeline) WriteAsync(entry *domain.LogEntry) bool {
    ...
    case BlockShort:
        ...
        timer := time.NewTimer(ap.blockFor)
        defer timer.Stop()
        select {
        case ap.logChan <- entry:
            return true
    ...
    case DropLatest:
        fallthrough
    default:
        select { case ap.logChan <- entry: return true; default: ap.dropped.Add(1); return false }
    }
}

func (ap *AsyncPipeline) start() { ap.wg.Add(1); go ap.batchWorker(); for i := 0; i < ap.workerCount; i++ { ap.wg.Add(1); go ap.sinkWorker() } }
func (ap *AsyncPipeline) batchWorker() {
    ...
    for {
        select {
        case <-ap.ctx.Done():
            if len(batch) > 0 { ap.flushBatch(batch) }
            return
        case entry := <-ap.logChan:
            batch = append(batch, entry)
            if len(batch) >= ap.batchSize { ap.flushBatch(batch); clear(batch); batch = batch[:0] }
        case <-ticker.C:
            if len(batch) > 0 { ap.flushBatch(batch); clear(batch); batch = batch[:0] }
        }
    }
}
...
func (ap *AsyncPipeline) flushBatch(batch []*domain.LogEntry) {
    ...
    select { case ap.batchChan <- copyBatch: ap.batches.Add(1); case <-ap.ctx.Done(): return }
    if ap.metricsHook != nil { ap.metricsHook(ap.Stats()) }
}
...
func (ap *AsyncPipeline) sinkWorker() {
    ...
    for {
        ...
        case batch := <-ap.batchChan:
            for _, e := range batch { for _, s := range ap.sinks { if s != nil { _ = s.Write(e) } } ; ap.processed.Add(1) }
            if ap.metricsHook != nil { ap.metricsHook(ap.Stats()) }
        }
    }
}
...
type AsyncStats struct{ Processed, Dropped, Batches, QueueSize uint64 }
func (ap *AsyncPipeline) Stats() AsyncStats { return AsyncStats{Processed: ap.processed.Load(), Dropped: ap.dropped.Load(), Batches: ap.batches.Load(), QueueSize: uint64(len(ap.logChan))} }
func (ap *AsyncPipeline) WriteSync(entry *domain.LogEntry) { for _, s := range ap.sinks { if s != nil { _ = s.Write(entry) } } }
...
type AsyncLogBrt struct { *UnifiedLogBrt; pipeline *AsyncPipeline }
...
func (al *AsyncLogBrt) Debugf(format string, args ...interface{}) { al.log(domain.DebugLevel, fmt.Sprintf(format, args...)) }
func (al *AsyncLogBrt) Infof(format string, args ...interface{})  { al.log(domain.InfoLevel, fmt.Sprintf(format, args...)) }
func (al *AsyncLogBrt) Warnf(format string, args ...interface{})  { al.log(domain.WarnLevel, fmt.Sprintf(format, args...)) }
func (al *AsyncLogBrt) Errorf(format string, args ...interface{}) { al.log(domain.ErrorLevel, fmt.Sprintf(format, args...)) }
func (al *AsyncLogBrt) Fatalf(format string, args ...interface{}) { al.log(domain.FatalLevel, fmt.Sprintf(format, args...)) }
...
func (al *AsyncLogBrt) SetMetricsHook(h func(AsyncStats)) { if al != nil && al.pipeline != nil { al.pipeline.SetMetricsHook(h) } }
...
func (al *AsyncLogBrt) SetBackpressurePolicy(policy AsyncBackpressurePolicy, blockFor time.Duration) {
    if al != nil && al.pipeline != nil { al.pipeline.policy, al.pipeline.blockFor = policy, blockFor }
}
