...
func (ol *OTelLogBrt) record(level domain.Level, start time.Time) {
    ...
    if c, ok := baseFields["context"].(context.Context); ok { metCtx = c }
    if ol.tp != nil {
...
func (ol *OTelLogBrt) Debug(msg string) { start := time.Now(); ol.Strategy.Debug(msg); ol.record(domain.DebugLevel, start) }
// Info logs a message at info level and records an OTEL metric.
func (ol *OTelLogBrt) Info(msg string)  { start := time.Now(); ol.Strategy.Info(msg); ol.record(domain.InfoLevel, start) }
func (ol *OTelLogBrt) Warn(msg string)  { start := time.Now(); ol.Strategy.Warn(msg); ol.record(domain.WarnLevel, start) }
func (ol *OTelLogBrt) Error(msg string) { start := time.Now(); ol.Strategy.Error(msg); ol.record(domain.ErrorLevel, start) }
func (ol *OTelLogBrt) Fatal(msg string) { start := time.Now(); ol.Strategy.Fatal(msg); ol.record(domain.FatalLevel, start) }
func (ol *OTelLogBrt) Debugf(f string, a ...interface{}) { start := time.Now(); ol.Strategy.Debugf(f, a...); ol.record(domain.DebugLevel, start) }
func (ol *OTelLogBrt) Infof(f string, a ...interface{})  { start := time.Now(); ol.Strategy.Infof(f, a...); ol.record(domain.InfoLevel, start) }
func (ol *OTelLogBrt) Warnf(f string, a ...interface{})  { start := time.Now(); ol.Strategy.Warnf(f, a...); ol.record(domain.WarnLevel, start) }
func (ol *OTelLogBrt) Errorf(f string, a ...interface{}) { start := time.Now(); ol.Strategy.Errorf(f, a...); ol.record(domain.ErrorLevel, start) }
func (ol *OTelLogBrt) Fatalf(f string, a ...interface{}) { start := time.Now(); ol.Strategy.Fatalf(f, a...); ol.record(domain.FatalLevel, start) }

// Fluent methods delegate and return the same wrapper (which implements base.LogBrt)
func (ol *OTelLogBrt) F(k string, v interface{}) LogBrt { ol.Strategy = ol.Strategy.F(k, v).(*strat.StrategyLogBrt); return ol }
func (ol *OTelLogBrt) Fs(m map[string]interface{}) LogBrt {
    ol.Strategy = ol.Strategy.Fs(m).(*strat.StrategyLogBrt); return ol
}
// Ctx attaches a context for correlation/propagation.
func (ol *OTelLogBrt) Ctx(ctx context.Context) LogBrt { ol.Strategy = ol.Strategy.Ctx(ctx).(*strat.StrategyLogBrt); return ol }
...
func (ol *OTelLogBrt) WithError(err error) LogBrt { if err == nil { return ol }; return ol.F("error", err.Error()) }
...
func (ol *OTelLogBrt) WithOnlyFields(m map[string]interface{}) LogBrt { ol.Strategy.Base = ol.Strategy.Base.WithOnlyFields(m); return ol }
func (ol *OTelLogBrt) Context() context.Context { if c, ok := ol.Strategy.Base.FieldsCopy()["context"].(context.Context); ok { return c }; return nil }
...
func (ol *OTelLogBrt) AddFilter(f port.FilterStrategy)   { if ol.Strategy != nil { ol.Strategy.AddFilter(f) } }
// AddSampler appends a sampler strategy.
func (ol *OTelLogBrt) AddSampler(s port.SamplerStrategy) { if ol.Strategy != nil { ol.Strategy.AddSampler(s) } }
// AddMasker appends a masker strategy.
func (ol *OTelLogBrt) AddMasker(m port.MaskingStrategy)  { if ol.Strategy != nil { ol.Strategy.AddMasker(m) } }
// RemoveStrategy removes a strategy by kind and name.
func (ol *OTelLogBrt) RemoveStrategy(t, n string)        { if ol.Strategy != nil { ol.Strategy.RemoveStrategy(t, n) } }
...
func (ol *OTelLogBrt) installPipelineMetricsHook() {
    if ol.tp == nil || ol.Strategy == nil { return }
    meter := ol.tp.Meter()
    if meter == nil { return }
    ...
    ol.Strategy.SetMetricsHook(func(s b.AsyncStats) {
        incN := func(name string, n uint64) {
            if n == 0 { return }
            if n > 1000 { meter.ObserveHistogram(context.Background(), name+"_delta", float64(n), labels); return }
            for i := uint64(0); i < n; i++ { meter.IncCounter(context.Background(), name, labels) }
        }
        if s.Processed >= prev.Processed { incN("obs_async_processed_total", s.Processed-prev.Processed) }
        if s.Dropped >= prev.Dropped   { incN("obs_async_dropped_total", s.Dropped-prev.Dropped) }
        if s.Batches >= prev.Batches   { incN("obs_async_batches_total", s.Batches-prev.Batches) }
        prev = s
