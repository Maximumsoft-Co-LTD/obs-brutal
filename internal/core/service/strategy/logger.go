...
func (sl *StrategyLogBrt) log(level domain.Level, msg string) {
    ...
    if processed == nil { return }
    if !sl.async.WriteAsync(processed) { sl.async.WriteSync(processed) }
    sl.Base.IncCount()
}
...
func (sl *StrategyLogBrt) Debugf(format string, args ...interface{}) { sl.log(domain.DebugLevel, fmt.Sprintf(format, args...)) }
func (sl *StrategyLogBrt) Infof(format string, args ...interface{})  { sl.log(domain.InfoLevel, fmt.Sprintf(format, args...)) }
func (sl *StrategyLogBrt) Warnf(format string, args ...interface{})  { sl.log(domain.WarnLevel, fmt.Sprintf(format, args...)) }
func (sl *StrategyLogBrt) Errorf(format string, args ...interface{}) { sl.log(domain.ErrorLevel, fmt.Sprintf(format, args...)) }
func (sl *StrategyLogBrt) Fatalf(format string, args ...interface{}) { sl.log(domain.FatalLevel, fmt.Sprintf(format, args...)) }

// Fluent chaining delegates to base
func (sl *StrategyLogBrt) F(key string, value interface{}) b.LogBrt { sl.Base = sl.Base.F(key, value).(*b.UnifiedLogBrt); return sl }
func (sl *StrategyLogBrt) Fs(fields map[string]interface{}) b.LogBrt {
    if len(fields) == 0 { return sl }
    sl.Base = sl.Base.Fs(fields).(*b.UnifiedLogBrt); return sl
}
func (sl *StrategyLogBrt) Ctx(ctx context.Context) b.LogBrt {
    if ctx == nil { return sl }
    sl.Base = sl.Base.Ctx(ctx).(*b.UnifiedLogBrt); return sl
}
...
func (sl *StrategyLogBrt) WithError(err error) b.LogBrt { if err == nil { return sl }; return sl.F("error", err.Error()) }
