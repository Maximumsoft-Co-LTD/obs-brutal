...
func (m *Manager) AddFilter(f port.FilterStrategy) { m.mu.Lock(); defer m.mu.Unlock(); m.filters = append(m.filters, f) }
// AddSampler appends a sampler to the pipeline.
func (m *Manager) AddSampler(s port.SamplerStrategy) { m.mu.Lock(); defer m.mu.Unlock(); m.samplers = append(m.samplers, s) }
// AddMasker appends a masker to the pipeline.
func (m *Manager) AddMasker(ms port.MaskingStrategy) { m.mu.Lock(); defer m.mu.Unlock(); m.maskers = append(m.maskers, ms) }
...
func (m *Manager) RemoveStrategy(kind, name string) {
    ...
    case "filter":
        ...
        for _, f := range m.filters { if f.Name() != name { nf = append(nf, f) } }
        m.filters = nf
    case "sampler":
        ns := make([]port.SamplerStrategy, 0, len(m.samplers))
        for _, s := range m.samplers { if s.Name() != name { ns = append(ns, s) } }
        m.samplers = ns
    case "masker":
        nm := make([]port.MaskingStrategy, 0, len(m.maskers))
        for _, ms := range m.maskers { if ms.Name() != name { nm = append(nm, ms) } }
        m.maskers = nm
    }
}
...
func (m *Manager) ProcessEntry(entry *domain.LogEntry) *domain.LogEntry {
    ...
    for _, f := range m.filters { if !f.ShouldLog(entry) { return nil } }
    for _, s := range m.samplers { if !s.ShouldSample(entry) { return nil } }
    if len(m.maskers) == 0 { return entry }
    ...
    for k, v := range entry.F { masked.F[k] = v }
    for _, ms := range m.maskers { masked.F = ms.MaskFields(masked.F) }
    return masked
}
