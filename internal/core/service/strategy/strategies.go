...
func NewLevelFilter(min, max domain.Level) port.FilterStrategy { return &LevelFilter{min: min, max: max} }
func (lf *LevelFilter) ShouldLog(entry *domain.LogEntry) bool { return entry != nil && entry.Level >= lf.min && entry.Level <= lf.max }
func (lf *LevelFilter) Name() string                         { return "level_filter" }
func (lf *LevelFilter) Configure(cfg map[string]interface{}) error {
    if v, ok := cfg["min"].(domain.Level); ok { lf.min = v }
    if v, ok := cfg["max"].(domain.Level); ok { lf.max = v }
    return nil
}

// RateSampler samples logs randomly by rate [0,1]
type RateSampler struct{ rate float64; rng *rand.Rand }

func NewRateSampler(rate float64) port.SamplerStrategy {
    if rate < 0 { rate = 0 } else if rate > 1 { rate = 1 }
...
func (rs *RateSampler) ShouldSample(_ *domain.LogEntry) bool {
    if rs.rng == nil { rs.rng = rand.New(rand.NewSource(time.Now().UnixNano())) }
...
func (as *AdaptiveSampler) ShouldSample(_ *domain.LogEntry) bool {
    r := as.base
    if r < as.min { r = as.min }
    if r > as.max { r = as.max }
...
func (as *AdaptiveSampler) Configure(cfg map[string]interface{}) error {
    if v, ok := cfg["base"].(float64); ok { as.base = v }
    if v, ok := cfg["min"].(float64); ok { as.min = v }
    if v, ok := cfg["max"].(float64); ok { as.max = v }
    return nil
}
...
func (rm *RegexMaskingStrategy) MaskFields(fields map[string]interface{}) map[string]interface{} {
    if len(rm.patterns) == 0 || len(fields) == 0 { return fields }
